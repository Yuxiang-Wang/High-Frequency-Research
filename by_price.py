# -*- coding: utf-8 -*-
"""
Created on Wed Sep 25 09:50:56 2019
...
#----------------------
summary = {}
#for con in conditions_price: # con = conditions_price[0]; cons_price[0]
for j in range(len(conditions_price)): # j=0
    con = conditions_price[j]
    cons = cons_price[j]
    fac_range = temp[con]
...
#    for i in range(6):
#        temp_range_noinf = range_noinf[np.log(range_noinf['resist_sell_lag'])<-i]
#        d[(-i,cons)] = np.corrcoef(np.log(temp_range_noinf['resist_sell_lag']),temp_range_noinf['ret_vwapmid_fur'])[0][1]
